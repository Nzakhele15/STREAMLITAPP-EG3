...
paths = {
    "tfidf_vectorizer": os.path.join(base_dir, 'models', 'TransformIDF_vectorizer.pkl'),
    "lr_model": os.path.join(base_dir, 'models', 'LogisticR_classifier_model.pkl'),
    "nb_model": os.path.join(base_dir, 'models', 'NaiveB_classifier_model.pkl'),
    "rf_model": os.path.join(base_dir, 'models', 'RandomF_classifier_model.pkl'),
    "svm_model": os.path.join(base_dir, 'models', 'SupportVM_classifier_model.pkl'),
    ...
    if choice == "Home":
        ...
        st.markdown(
            ...
            - **Distribution Analysis:** 
              - **Category Distribution:** Explore the distribution of news articles across different categories to understand the prevalence and imbalance of categories in the dataset.
              - **Word Frequency Analysis:** Identify the most common words and phrases used in each category, which helps in understanding the content and themes prevalent in different types of news.
              - **Length Analysis:** Examine the length of articles in terms of word count and character count across different categories to identify any notable differences.

            - **Labeling and Dataset Characteristics:** 
              - **Labeled Dataset:** The dataset consists of news articles labeled into categories such as sports, education, entertainment, business, and technology. These labels are essential for training supervised machine learning models.
              - **Source and Timeframe:** Information about the source of the dataset and the timeframe over which the articles were collected can provide context for the analysis.

            - **Exploratory Data Analysis (EDA):**
              - **Pattern Identification:** Use EDA to uncover patterns, trends, and anomalies in the data that can inform feature engineering and model selection.
              - **Bias Detection:** Detect and analyze biases in the dataset, such as over-representation or under-representation of certain categories, which can impact model performance.
              - **Correlation Analysis:** Investigate correlations between different features (e.g., publication date, article length) and the target categories.
              - **Sentiment Analysis:** Perform sentiment analysis to understand the emotional tone of articles across different categories and how it may affect classification.
              - **Visualization:** Use visualizations such as histograms, bar charts, word clouds, and box plots to present the data in an intuitive and insightful manner.
              - **Missing Values:** Check for missing values and understand their distribution and impact on the dataset.

            - **Model Development:**
              - **Feature Selection:** EDA helps in selecting the most relevant features for model development by identifying important patterns and correlations.
              - **Data Cleaning:** Identify and address any data quality issues such as duplicates, inconsistencies, or errors in the dataset.
              - **Balancing the Dataset:** Techniques such as oversampling, undersampling, or using synthetic data generation (e.g., SMOTE) to balance the dataset and improve model performance.

            - **Model Evaluation:**
              - **F1 Scores:** Evaluate the performance of different models using F1 scores, which provide a balance between precision and recall. F1 scores are particularly useful for imbalanced datasets.
              ...
              - **ROC and AUC:** Examine the Receiver Operating Characteristic (ROC) curve and Area Under the Curve (AUC) for each model to evaluate their discriminatory power.
              - **Precision-Recall Curve:** Investigate the precision-recall curve to assess the trade-off between precision and recall at different threshold levels.

            - **Correlation Matrix:**
              - **Feature Correlation:** Generate and analyze the correlation matrix to understand the relationships between different features. This helps in identifying multicollinearity and selecting features that contribute the most to the model's performance.

            - **Training Time:**
              - **Model Training Duration:** Measure and compare the training time for different models to understand their computational efficiency and scalability.
              - **Optimization:** Explore optimization techniques to reduce training time without compromising model accuracy, such as using faster algorithms, reducing data dimensionality, or parallelizing computations.
            """
        )
        ...
        st.markdown(
            ...
            Your feedback is essential for us to improve this application. Please share your comments, suggestions, and any issues you've encountered. Whether it's about the user interface, functionality, or any new features you'd like to see, we are eager to hear from you.
        ...
        st.markdown(
            ...
            ### Acknowledgments
            We extend our gratitude to our supervisors for their invaluable guidance and support throughout this project. Special thanks to ExploreAI Academy for providing us with this opportunity to apply our skills in a real-world scenario.

            ### Conclusion
            We achieved our goal and created a transformative tool designed to enhance the way you interact with the world of news. It prioritizes your preferences and curates content that truly matters to you. We are redefining the news consumption experience.
            ...
            **Join Us** in our mission to bring innovative data solutions to the world. Follow our journey and contribute to the future of news classification technology.
